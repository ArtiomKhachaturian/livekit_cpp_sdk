...
webrtc::RTCErrorOr<std::unique_ptr<VTDecoderSession>> VTDecoderSession::
    ...
    if (format) {
        ...
        if (const auto imageAttrs = sourceImageAttributes(outputPixelFormat, dimensions.width, dimensions.height)) {
            ...
            if (noErr == status) {
                ...
                return std::unique_ptr<VTDecoderSession>(new VTDecoderSession(std::move(pipeline),
                                                                              std::move(format),
                                                                              session,
                                                                              hardwareAccelerated));
            }
...
{
    return _pipeline->pendingFramesCount();
}

webrtc::RTCError VTDecoderSession::lastOutputStatus() const
{
    return toRtcError(_pipeline->lastOutputStatus());
}
                                              ...
                                              CMTime timestamp, CMTime duration)
{
    ...
    if (auto selfRef = reinterpret_cast<DecodePipeline*>(pipeline)) {
        ...
        if (noErr == status) {
            ...
            selfRef->invokeCallback(&VTDecoderSessionCallback::onDecodedImage, timestamp,
                                    duration, infoFlags,
            ...
            selfRef->invokeCallback(&VTDecoderSessionCallback::onError, status, true);
        }
    }
}
...
void VTDecoderSession::DecodePipeline::adjustPresentationTimestamp(CMSampleBufferRef encodedBuffer, uint32_t timestamp) const
{
    if (encodedBuffer && 0 == CMTimeCompare(kCMTimeInvalid, CMSampleBufferGetOutputPresentationTimeStamp(encodedBuffer))) {
        ...
        if (!status) {
            invokeCallback(&VTDecoderSessionCallback::onError, status, false);
        }
    }
}
