...
public:
    ...
    // impl. of KeyProvider
    bool setSharedKey(std::vector<uint8_t> key, const std::optional<size_t>& keyIndex = {}) final;
    std::shared_ptr<ParticipantKeyHandler> sharedKey(const std::string& participantId) final;
    std::vector<uint8_t> ratchetSharedKey(const std::optional<size_t>& keyIndex = {}) final;
    std::vector<uint8_t> exportSharedKey(const std::optional<size_t>& keyIndex = {}) const final;
    bool setKey(const std::string& participantId, std::vector<uint8_t> key,
                const std::optional<size_t>& keyIndex = {}) final;
    ...
    std::vector<uint8_t> ratchetKey(const std::string& participantId,
                                    const std::optional<size_t>& keyIndex = {}) const final;
