...
public:
    ...
    // track, it is called immediately if the first packet was already sent
    virtual void onFirstFrameSent(const std::string& /*id*/) {}
    // only for local tracks, called when the first media packet is received for the observed
    // track, it is called immediately if the first packet was already received
    virtual void onFirstFrameReceived(const std::string& /*id*/) {}
